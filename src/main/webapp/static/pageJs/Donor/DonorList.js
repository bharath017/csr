$(document).ready(function(){
	...
	var table =   $('#district-table').DataTable({
        ...
        "columnDefs":[
        	...
        	"render":function(data){
        		return '<b><a href="#">'+data.donor_id+'</a></b>'
        	}},
        	{"targets":[1],"data":"donor_name"},
        	{"targets":[2],"data":"donor_address"},
            {"targets":[3],"data":"donor_contact_person_name"},
            {"targets":[4],"data":"donor_contact_person_phone"},
        	...
        	"render":function(data,row){
        		 let html = `
        		 	...
        		 	&nbsp;&nbsp;&nbsp;
        		 	<a id="${data.district_id}" name="${data.district_name}" class="delete-district" style="cursor:pointer;">
	...
	//for delete option
	$(document).on('click','#district-table .delete-district',function(){
		console.log('cliking');
		let district_id = $(this).attr('id');
		let district_name = $(this).attr('name');
		swal(
				{title:`Are you sure want to delete District ${district_name}?`,
				text:`You can\'t recover this District ${district_name}!`,
				type:"warning",
				showCancelButton:!0,
				...
				function(){
					fetch(`${ctx}/Region/deleteDistrict/${district_id}`,{
						method:'delete',
						headers:{
							'X-CSRF-TOKEN':$('input[name="_csrf"]').val()
						}
